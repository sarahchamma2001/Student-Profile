...
#conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=C:\Users\sarah\Desktop\New folder\OSB_Faculty_Database_F1.accdb')
conn=pd.read_csv("Student.csv",encoding='ISO-8859-1')

conn1 = pd.read_csv("Studentprofile.csv",encoding='ISO-8859-1')
...
if user_options == "2023" or user_options == "2021" or user_options == "2022":
    ...
    menu = option_menu(None, ["Demographics",  "Analysis", "GPA"], icons=["search", "bar-chart-line", "clipboard-check"],
                       ...
                       styles={"container": {"padding": "0!important", "background-color": "#fafafa"},
                               ...
                               "nav-link-selected": {"background-color": "rgba(128, 0, 32, 0.8)"}, })
#2023
conn1= conn1.drop_duplicates(subset="Id Number")
#####################################
conn1["Full Name"] = conn1["First Name"]+" "+conn1["Last Name"]
conn1 = conn1.set_index("Full Name")

df_2023_MSBA = conn1[conn1["student Type"] == "MSBA"]
df_2023_MSBA["Full Name"] = df_2023_MSBA["First Name"] +" "+df_2023_MSBA["Last Name"]
#####################################
df_2023_MFIN = conn1[conn1["student Type"] == "MF"]
df_2023_MFIN["Full Name"] = df_2023_MFIN["First Name"] +" "+df_2023_MFIN["Last Name"]
#####################################
df_2023_MHRM = conn1[conn1["student Type"] == "MHRM"]
df_2023_MHRM["Full Name"] = df_2023_MHRM["First Name"] +" "+df_2023_MHRM["Last Name"]
#####################################
df_2023_MSBA_FT = conn1[conn1["category"] == "FT"]
df_2023_MSBA_PT = conn1[conn1["category"] == "PT"]
#####################################
...
def table1(dataframe):
    ...
    for i in range(l):
        ...
        with col1:
            if x < len(dataframe):
                if os.path.isfile(rf"{dataframe.iloc[x,1]}.jpg"):
                    image = Image.open(rf"{dataframe.iloc[x,1]}.jpg")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x,1]}.jpeg"):
                    image = Image.open(rf"{dataframe.iloc[x,1]}.jpeg")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x,1]}.png"):
                    image = Image.open(rf"{dataframe.iloc[x,1]}.png")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x,1]}.jfif"):
                    image = Image.open(rf"{dataframe.iloc[x,1]}.jfif")
                    st.image(image, output_format="JPEG", width=165)
                else:
                    backup = Image.open(rf"{p}")
                    st.image(backup, output_format="JPEG", width=165)

                with st.expander(dataframe.iloc[x, 11]):
                    st.write(f"**ID:** {dataframe.iloc[x,1]}")
                    st.write(f"**Status:** {dataframe.iloc[x,6]}")
                    st.write(f"**E-mail:** {dataframe.iloc[x,4]}")
                    st.write(f"**LinkedIn:** {dataframe.iloc[x,10]}")
                    st.write(f"**Bio:** {dataframe.iloc[x,5]}")

        with col2:
            if x+1 < len(dataframe):
                if os.path.isfile(rf"{dataframe.iloc[x+1,1]}.jpg"):
                    image = Image.open(rf"{dataframe.iloc[x+1,1]}.jpg")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x+1,1]}.jpeg"):
                    image = Image.open(rf"{dataframe.iloc[x+1,1]}.jpeg")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x+1,1]}.png"):
                    image = Image.open(rf"{dataframe.iloc[x+1,1]}.png")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x+1,1]}.jfif"):
                    image = Image.open(rf"{dataframe.iloc[x+1,1]}.jfif")
                    st.image(image, output_format="JPEG", width=165)
                else:
                    backup = Image.open(rf"{p}")
                    st.image(backup, output_format="JPEG", width=165)
  
                with st.expander(dataframe.iloc[x+1, 11]):
                    st.write(f"**ID:** {dataframe.iloc[x+1,1]}")
                    st.write(f"**Status:** {dataframe.iloc[x+1,6]}")
                    st.write(f"**E-mail:** {dataframe.iloc[x+1,4]}")
                    st.write(f"**LinkedIn:** {dataframe.iloc[x+1,10]}")
                    st.write(f"**Bio:** {dataframe.iloc[x+1,5]}")

        with col3:
            if x+2 < len(dataframe):
                if os.path.isfile(rf"{dataframe.iloc[x+2,1]}.jpg"):
                    image = Image.open(rf"{dataframe.iloc[x+2,1]}.jpg")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x+2,1]}.jpeg"):
                    image = Image.open(rf"{dataframe.iloc[x+2,1]}.jpeg")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x+2,1]}.png"):
                    image = Image.open(rf"{dataframe.iloc[x+2,1]}.png")
                    st.image(image, output_format="JPEG", width=165)
                elif os.path.isfile(rf"{dataframe.iloc[x+2,1]}.jfif"):
                    image = Image.open(rf"{dataframe.iloc[x+2,1]}.jfif")
                    st.image(image, output_format="JPEG", width=165)
                else:
                    backup = Image.open(rf"{p}")
                    st.image(backup, output_format="JPEG", width=165)
                with st.expander(dataframe.iloc[x+2, 11]):
                    st.write(f"**ID:** {dataframe.iloc[x+2,1]}")
                    st.write(f"**Status:** {dataframe.iloc[x+2,6]}")
                    st.write(f"**E-mail:** {dataframe.iloc[x+2,4]}")
                    st.write(f"**LinkedIn:** {dataframe.iloc[x+2,10]}")
                    st.write(f"**Bio:** {dataframe.iloc[x+2,5]}")

        x = x+3
#2023 MBA & OMBA
conn= conn.drop_duplicates(subset="Id Number")

df_ss_2023 = conn[conn["yearofadmission"].isin([202310, 202320])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_s_23 = conn.set_index("Full Name")
...
#2022
# FT or PT 2022
df_2022 = conn[conn["yearofadmission"].isin([202210, 202220])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_22 = conn.set_index("Full Name")
        ...
        with col1:
            if x < len(dataframe):
                ...
                if os.path.isfile(backup_path):
                    ...
                    st.image(backup, output_format="JPEG", width=165)
                    with st.expander(dataframe.iloc[x, 10]):
                        st.write(f"**ID:** {dataframe.iloc[x,1]}")
                        st.write(f"**Status:** {dataframe.iloc[x,5]}")
                        st.write(f"**E-mail:** {dataframe.iloc[x,4]}")
                        st.write(f"**LinkedIn:** {dataframe.iloc[x,9]}")
        ...
        with col2:
            if x+1 < len(dataframe):
                ...
                if os.path.isfile(backup_path):
                    backup = Image.open(backup_path)
                    st.image(backup, output_format="JPEG", width=165)
                    with st.expander(dataframe.iloc[x+1, 10]):
                        st.write(f"**ID:** {dataframe.iloc[x+1,1]}")
                        st.write(f"**Status:** {dataframe.iloc[x+1,5]}")
                        st.write(f"**E-mail:** {dataframe.iloc[x+1,4]}")
                        st.write(f"**LinkedIn:** {dataframe.iloc[x+1,9]}")
        with col3:
            if x+2 < len(dataframe):
                backup_path = rf"{p}"
                if os.path.isfile(backup_path):
                    backup = Image.open(backup_path)
                    st.image(backup, output_format="JPEG", width=165)
                    with st.expander(dataframe.iloc[x+2, 10]):
                        st.write(f"**ID:** {dataframe.iloc[x+2,1]}")
                        st.write(f"**Status:** {dataframe.iloc[x+2,5]}")
                        st.write(f"**E-mail:** {dataframe.iloc[x+2,4]}")
                        st.write(f"**LinkedIn:** {dataframe.iloc[x+2,9]}")

        x = x+3
# 2018

df_2018 = conn[conn["yearofadmission"].isin([201810, 201820])]

conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_18 = conn.set_index("Full Name")
...
df_2019 = conn[conn["yearofadmission"].isin([201910, 201920])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_19 = conn.set_index("Full Name")
...
df_2020 = conn[conn["yearofadmission"].isin([202010, 202020])]

conn["Full Name"] = conn["First Name"] +  " "+conn["Last Name"]
df_search_20 = conn.set_index("Full Name")
...
# 2021
df_2021 = conn[conn["yearofadmission"].isin([202110, 202120])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_21 = conn.set_index("Full Name")
...
# 2017
df_2017 = conn[conn["yearofadmission"].isin([201710, 201720])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_17 = conn.set_index("Full Name")
...
df_2016 = conn[conn["yearofadmission"].isin([201610, 201620])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_16 = conn.set_index("Full Name")
...
# 2015
df_2015 = conn[conn["yearofadmission"].isin([201510, 201520])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_15 = conn.set_index("Full Name")
...
df_2014 = conn[conn["yearofadmission"].isin([201410, 201420])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_14 = conn.set_index("Full Name")
...
df_2013 = conn[conn["yearofadmission"].isin([201310, 201320])]
conn["Full Name"] = conn["First Name"] +" "+conn["Last Name"]
df_search_13 = conn.set_index("Full Name")
...
d= conn.drop_duplicates(subset="Id Number")
...
if user_options == "Advanced Option":
    ...
    if Years == "2023":
        def search(dataframe):
            ...
            with st.container():
                if os.path.isfile(rf"{id}.jpg"):
                    image = Image.open(rf"{id}.jpg")
                    st.image(image, output_format="JPEG", width=200)
                elif os.path.isfile(rf"{id}.jpeg"):
                    image = Image.open(rf"{id}.jpeg")
                    st.image(image, output_format="JPEG", width=200)
                elif os.path.isfile(rf"{id}.png"):
                    image = Image.open(rf"{id}.png")
                    st.image(image, output_format="JPEG", width=200)
                elif os.path.isfile(rf"{id}.jfif"):
                    image = Image.open(rf"{id}.jfif")
                    st.image(image, output_format="JPEG", width=200)
                else:
                    backup = Image.open(
            ...
            with st.container():
                ...
                v1.metric("Linkedin:", dataframe.loc[name, "linkedurl"])
            with st.container():
